"""Telegram handlers: download voice message → transcribe → reply."""
...
_START_TEXT = (
    "Hi! Send or forward me a voice message and I'll reply with a text transcription."
)
...
class VoiceHandler:
    ...
    async def handle(self, update, context) -> None:
        ...
        MESSAGES_RECEIVED.labels(type="voice").inc()
        ...
        try:
            ...
            log.info(
                "transcribed chat=%s msg=%s lang=%s audio_s=%.1f chars=%d",
                chat_id, msg.message_id, result.language, duration, len(result.text),
            )
